...
public:
    ...
    */
    explicit shm_hub(ygm::comm &c, uint32_t topk, uint32_t num_edges,
        ...
        static std::vector<std::pair<uint64_t, size_t>> topk_hubs;
        ...
        while(cumulative_edges < m_num_edges && topk_hub_curr != topk_hubs.end()){
        ...
        ygm::container::bag<Edge> bag_nonhub_edges(m_comm);
        ygm::container::bag<Edge> bag_hub_edges(m_comm);
        bagbp->for_all([this, &bag_nonhub_edges, &bag_hub_edges](const Edge& ed){
        ...
        nonhub_edges = std::make_unique<ygm::container::array<Edge>>(m_comm, bag_nonhub_edges);
        static std::vector<Edge> hub_edges;
        bag_hub_edges.gather(hub_edges, 0);
        if(m_comm.rank0()){
            for(int i = m_local_size; i < m_comm.size() ; i += m_local_size){
                m_comm.async(i, [](std::vector<Edge> edges){
                    hub_edges = edges;
                }, hub_edges);
            }
        }
        m_comm.barrier();
        if(m_local_id == 0){
            m_comm.cout("I am master rank. I received hub edges of size: ", hub_edges.size());
        }
        m_comm.cout0("Hub pre-processing time: ", MPI_Wtime() - hub_processing_time);
        bagbp.reset();
        ...
        // std::string filename_s = "/BIP_" + std::to_string(m_node_id);
    ...
    //     int slot = ygm::container::detail::hash<key_type>{}(key) % m_num_edges;
    //     YGM_ASSERT_DEBUG(BIP_index < m_local_size);
    //     YGM_ASSERT_DEBUG(slot < m_num_edges);
